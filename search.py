...
def generalTreeSearch(problem,fringe):
    ...
    def addSuccessorToFringe(start,cost):
        for next, action, added_cost in problem.getSuccessor(start):
            if hash(next) not in visited: # If there is not a hash key of value next  in our dictionary
    ...
    while not fringe.isEmpty():
        ...
        addSuccessorToFringe(next,cost)
...
def uniformCostSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    def fringeForItem(item):
        state_s, state_e, action, cost = item
        return cost
    fringe = util.PriorityQueueWithFunction(fringeForItem)
